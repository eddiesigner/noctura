...
Technical documentation for working on Noctura. For what the extension
...
src/
  entrypoints/
    ...
    content.ts                Applies/removes the dark styles, handles the shortcut/messages, owns per-site overrides
  ...
  composables/
    ...
    usePageToggle.ts           Popup's per-tab on/off toggle (live state, override status)
...
- With "Match system dark mode" or "Scheduled dark mode" on, toggle a
  specific site anyway — it should still take effect immediately (a
  per-site override), the popup should show a "Reset to automatic" link
  for that site, and it should survive a reload. Click that link (or
  forget the site from Settings) to confirm it falls back to the
  automatic value.
...
## Automatic modes, in detail

### Match system dark mode

Off by default, in Settings or the popup. When it's on, it governs every
page's dark mode by default: a page is dark whenever the OS reports
`prefers-color-scheme: dark`, and light otherwise. A manual toggle (popup
or shortcut) on a specific site still works — it creates a per-site
override (see `content.ts`'s `applyState`/`persistState`) that takes
priority over the automatic value for that site, independent of the
"Remember per website" setting, until it's cleared.
...
the page loads, the setting changes, or a manual toggle recomputes state.
...
"governs every page by default, per-site override still available"
behavior as Match system dark mode, and the two are mutually exclusive
(turning one on turns the other off and disables its switch). "Remember
per website" is locked while either automatic mode is on, since manual
toggling in that state creates an override instead of feeding that
setting's normal-mode memory.
...
catch up next time it's focused or reloaded. A per-site override is
re-applied on every one of these rechecks, so it isn't clobbered by them.
